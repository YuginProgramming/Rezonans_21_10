...
const recordDay_ukr = {
    reply_markup: JSON.stringify({
        inline_keyboard: [
            ...
            [{text: 'Середа', callback_data: 'Среда'}, {text: 'Четвер', callback_data: 'Четверг'}],
...
//Itaration ONE
        if(text === '/start') {
            ...
            await bot.sendMessage(chatId, 'Виберіть будьласка мову - Сделайте выбор языка', language);
            return;
        // 
        }
//Contacts reply        
...
let marker4 = false;
    bot.on('callback_query', async msg => {
        ...
        if (Number(msg.data) >= 9 && Number(msg.data <= 18)) {
            ...
            await bot.sendMessage(chatId, `${e}, ${FIO}, ${f} в ${day} ${msg.data}.00`);
            chat[1] = `${msg.data}.00`;
